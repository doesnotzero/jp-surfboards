...
const socialCardPath = resolve(root, 'public/social-card.jpg');
const socialCardUrl = 'https://www.jpsurfboards.com.br/social-card.jpg';
...
function getJpegSize(path) {
  ...
  assert(buffer[0] === 0xff && buffer[1] === 0xd8, 'social-card.jpg is not a JPEG');
...
assert(getMetaContent('property', 'og:image') === socialCardUrl, 'og:image must use social-card.jpg');
assert(
  getMetaContent('property', 'og:image:secure_url') === socialCardUrl,
  'og:image:secure_url must use social-card.jpg',
);
assert(getMetaContent('name', 'twitter:image') === socialCardUrl, 'twitter:image must use social-card.jpg');
...
assert(width === 1200 && height === 630, `social-card.jpg must be 1200x630, got ${width}x${height}`);

const sizeInMb = statSync(socialCardPath).size / 1024 / 1024;
assert(sizeInMb < 1, `social-card.jpg should stay below 1MB, got ${sizeInMb.toFixed(2)}MB`);
